# ADR-001: study-leader-skill を projects/leader-skill/ へ吸収する
...
- `study-leader-skill` を PaC リポジトリの `projects/leader-skill/` へ **git subtree マージ（履歴保持）** で吸収する。
- PaC を親、`projects/leader-skill/` を子とし、ハーネス（`.claude/`）・秘密情報管理（`env/` + Doppler）・コミット規約は親に一本化する。子配下に独自の Makefile / env / doppler.yaml / AGENTS.md は置かない。
- 子固有の作業ルールは `projects/leader-skill/CLAUDE.md` に置き、親 `CLAUDE.md` と併読する。
...
- **src/ 配下へ配置**: `src/` はアプリケーションコードの置き場。子プロジェクトは複数化しうるため `projects/` を新設。

## Consequences

- 今後の資格戦略・教材の更新はすべて `projects/leader-skill/` で行い、PaC のタスク運用（`docs/tasks/`）・判断記録（`docs/decisions/`）に乗せる。
