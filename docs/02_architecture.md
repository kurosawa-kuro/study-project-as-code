...
本リポジトリ自身の成熟度は次の4段階で管理する（マチュリティモデル）。

| フェーズ | 状態 | 完了条件 |
|---|---|---|
| 1. 骨格 | 位置づけ・価値判断基準・運用資産・ハーネスが揃っている | ADR-002/003/004、優先順位15項目、framework-stack 一式、`.claude/skills/`（**2026-07-07 完了**） |
| 2. 実戦1つ | 実プロジェクトを1つ PaC で回し、型の過不足を実負荷で検証する | 1プロジェクトが PaC の型（task / WBS / RAID / KPI / Issue as Code）で開始〜完了し、型への修正がフィードバックされている |
| 3. 資産再利用 | 2つ目のプロジェクトで1つ目の資産を再利用する | `profiles/` に最初の役割別プリセットが実績から生まれている |
| 4. 持ち出し可能 | 他リポ・実案件へ PaC 一式を展開できる | `src/project-as-code/` が導入手順つきで外部適用されている |

**現在位置: フェーズ2進行中（2026-07-07〜、最初の実戦 = CAPM 挑戦。`docs/tasks/active/2026-07-07-capm-pac-live-run.md`）。**

フェーズ2以降で守ること:

- 実負荷なしに骨格を磨き続けない。メタ作業の継続は `src/project-as-code/src/tailoring-policy.md` の破綻条件（全部盛り・台帳化）へ向かう。
- `profiles/` や `generated/` などの器は、フェーズを先取りして作らない（実績から埋める）。
- フェーズ判定の変更はこの表を更新し、判断は decision-log に残す。
